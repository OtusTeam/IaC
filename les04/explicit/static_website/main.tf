...
resource "yandex_storage_bucket_iam_binding" "storage_admin" {
  ...
  # но только после завершения бакета s3:
...
resource "yandex_storage_bucket_grant" "public_read" {
  ...
  secret_key = yandex_iam_service_account_static_access_key.sa_static_key.secret_key
  acl        = "public-read"
  # но только после назначения сервисного эккаунта администратором бакета s3
...
resource "null_resource" "curl" {
  ...
  # но только после разрешения публичного доступа на чтения к бакету s3: 
