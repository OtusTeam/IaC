...
token = ycfg.require("token")
cloud_id = ycfg.require("cloudId")
...
# Параметры
bucket_name = f"{prefix}-s3-{pulumi.get_stack()}"
# pulumi.Config().get("bucketName")

# Выполнить HTTP-запрос при 'pulumi up'
resp = requests.get(from_url)	# https://jsonplaceholder.typicode.com/posts ?

resp.raise_for_status()
items = resp.json()[:3]

# Создать сервисный аккаунт для доступа к Object Storage
